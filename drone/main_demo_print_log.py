...
def control_drone(speed = 0.5):
    ...
    if shm_foreign_object_flag == 1:
        speed = 0.2
        if abs(heading) >= 3 and abs(offset) >= 0.1:   
            print(f'anomaly detected || go straight amd roll {-1*offset} and yaw {-1*heading}')
            ...
            print(f"anomaly detected || go straight and yaw {-1*heading}")
            # time.sleep(1)
        elif abs(heading) < 3 and abs(offset) >= 0.1:
            # Drone.control_forward_and_roll(P*offset, speed)
            print(f"anomaly detected || go straight and roll {-1*offset}")
            # time.sleep(1)
        elif abs(heading) < 3 and abs(offset) < 0.1:
            # Drone.control_only_vel_xyz(speed, 0, 0)
            print("anomaly detected || go straight")
            # time.sleep(1)   
